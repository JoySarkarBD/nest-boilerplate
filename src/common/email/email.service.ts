...
export class EmailService {
   ...
   */
  async sendEmail(dto: SendEmailDto): Promise<{ queued: number; message: string }> {
    ...
    this.logger.log(`Queued ${recipients.length} email job(s) → subject: "${dto.subject}"`);

    return { queued: recipients.length, message: 'Email(s) queued successfully' };
  }
   ...
   */
  async sendBulkEmail(dtos: SendEmailDto[]): Promise<{ queued: number; message: string }> {
    let total = 0;
